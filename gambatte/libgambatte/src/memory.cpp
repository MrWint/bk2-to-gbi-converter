...
void Memory::updateInput(unsigned long const cc) {
	unsigned state = 0xF;

	if ((ioamhram_[0x100] & 0x30) != 0x30 && getInput_) {
		unsigned input = (*getInput_)(getInputContext_, (cc - basetime_) >> (1 + isDoubleSpeed()));
...
void Memory::nontrivial_ff_write(unsigned const p, unsigned data, unsigned long const cc) {
	...
	case 0x00:
		if ((data ^ ioamhram_[0x100]) & 0x30) {
			...
			// updateInput(); joypad interrupts unsupported
		}
